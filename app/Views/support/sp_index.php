...
<head>
    ...
    <style {csp-style-nonce}>
        ...
        .sp_main_notice {
            position: absolute; /* 슬라이드 각 요소들을 절대적 위치로 배치 */
            top: 0; /* 상단에 위치 */
            left: 0; /* 왼쪽에 위치 */
            width: 100%; /* 부모 요소에 꽉 차도록 */
            opacity: 0; /* 초기에 투명하게 설정 */
            transition: opacity 1s ease; /* opacity가 변할 때 애니메이션 적용 */
        }
        .sp_main_notice.active {
            opacity: 1; /* 활성화된 슬라이드는 투명도를 1로 설정하여 보이도록 함 */
        }
    ...
    <body style="max-width: 400px; margin: 0 auto;position: relative;">
        ...
        ?>
            ...
            <body>
                ...
                <footer class="footer <?php if (!$ci) { echo "block";} ?>" style="background:#000;">
